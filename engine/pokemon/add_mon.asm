...
.next2
    ...
    and a
    jr nz, .skipNaming
    ; Remove or bypass the naming screen setup and call
    ...
    ; predef AskName
.skipNaming
    ; Copy the default name pointer from the species data
    ld hl, wPartyMonNicks
    ldh a, [hNewPartyLength]
    dec a
    call SkipFixedLengthTextEntries
    ...
    ; Check if the name returned (in wStringBuffer) is "blank"
    ld a, [wStringBuffer]
    cp "@"                    ; assuming '@' indicates a blank/default result
    jr z, .useWNameBuffer     ; if equal, use original name in wNameBuffer

    ; Otherwise, copy the name from wStringBuffer
    ld hl, wStringBuffer
    ld bc, NAME_LENGTH
    call CopyData
    jr .skipNamingDone
...
.useWNameBuffer:
    ; Copy the original/default name from wNameBuffer
    ld hl, wNameBuffer
    ld bc, NAME_LENGTH
    call CopyData
